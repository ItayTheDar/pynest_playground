...
class PyNestContainer:
    ...
    def _get_controllers(self, token: str):
        ...
        controllers = self.extract_module_param_metadata(
            module_metadata, CONTROLLERS_TOKEN
        )
        return controllers
    ...
    def set_module(self, module_factory: ModuleFactory) -> Module:
        ...
        self.logger.info(
            f"{click.style( module_factory.type.__name__ + ' Loaded', fg='green')}",
        )
        return module_ref
    ...
    def add_import(self, token: str):
        ...
        imports_mod: List[Any] = self.extract_module_param_metadata(
            module_metadata, IMPORTS_TOKEN
        )
    ...
    def extract_module_param_metadata(
        self, module_metadata: Dict[str, List], params_name: str
    ):
